...
function sendMail() {
    var params = {
        ...
        // You can add more fields later, e.g. subject: ..., phone: ...
    };

    const serviceID = "service_ivfdfcl";      // ← Replace with YOUR Service ID
    const templateID = "template_ybiks87";    // ← Replace with YOUR Template ID
    ...
    emailjs.send(serviceID, templateID, params)
        .then(
            function (response) {
                ...
                // Reset button (in case user clicks OK and tries again)
                contactButton.classList.remove('loading');
                contactLoad.classList.remove('show');
                submitText.classList.remove('hide');
            },
